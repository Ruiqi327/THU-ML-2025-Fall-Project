...
def generate_dataset_linear(type='linear', num_samples=1000, num_features=2, output_dir='data'):
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    X = np.random.rand(num_samples, num_features)
    X[:,0]= (X[:,0]*8+1)  # Scale learning time to [1,8]
    X[:,1]= (X[:,1]*70+30)  # Scale midterm grade to [30,100]
    ...
    if type == 'linear':
        filename = '2f-linear_dataset.csv'
        for i in range(num_samples):
            y_label[i] = 1 if 0.5 < y[i] else -1
    elif type == 'nonlinear':
        filename = '2f-nonlinear_dataset.csv'
        for i in range(num_samples):
            y_label[i] = 1 if 0.25 < y[i] < 0.75 else -1
    else:
        filename = '2f-noisy_dataset.csv'
        for i in range(num_samples):
    ...
    headers = "learning_time,midterm_grade,pass_fail"
    np.savetxt(output_path, combined_dataset, delimiter=',', header=headers, comments='', fmt='%.5f')

def main():
    parser = argparse.ArgumentParser(description="2 feature dataset")
